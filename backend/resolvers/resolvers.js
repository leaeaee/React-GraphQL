...
const resolvers = {
  Query: {
    ...
    login: async (_, { email, password }) => {
      ...
      const token = jwt.sign({ userId: user.id, email: user.email }, 'somesupersecretkey', { expiresIn: '10h' });
  ...
  Mutation: {
    ...
    removeFromFavorites: async (_, { favoritesId }, { currentUserId }) => {
      ...
      try {
        ...
        console.log('delete, result favourites', res);
        return favoritesId;
      } catch (err) {
        throw err;
      }
    ...
    deleteProduct: async (_, args) => {
      ...
      try {
        const deletedProduct = await Product.findById(args.productId).populate('product');
